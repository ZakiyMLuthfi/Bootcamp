  ...
  if (val.isAlpha(nama) && val.isMobilePhone(nomor) && val.isEmail(email)) {
    ...
    if (!val.isAlpha(nama)) {
      ...
      if (!val.isMobilePhone(nomor)) {
        ...
        if (!val.isEmail(email)) console.log("Email kamu salah");
        return main();
      }
    }
  }
...
function saveData(data) {
  try {
    ...
    console.log(
      "Terjadi kesalahan saat menyimpan data. Periksa kembali data yang anda berikan"
    );
  }
}
  ...
  rl.question("Sebutkan nama: ", (nama) => {
    rl.question("Sebutkan nomor: ", (nomor) => {
      rl.question("Sebutkan email: ", (email) => {
        ...
        if (newData) {
          rl.question(
            "Apakah anda ingin menyimpan data? (ya/tidak): ",
            (answer) => {
